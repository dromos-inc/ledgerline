// AR aging report + sub-ledger reconciliation canary banner.
...
export function ArAging({ companyId }: Props) {
  const svc = reportsApi(companyId);
  const today = new Date().toISOString().slice(0, 10);
  const [asOfDate, setAsOfDate] = useState(today);

  const { data: aging, isLoading } = useQuery<ArAgingReport>({
    queryKey: ["ar-aging", companyId, asOfDate],
    queryFn: () => svc.arAging(asOfDate),
  });
  ...
  const drift = recon ? recon.ar_difference_cents : 0;
    ...
    <div className="p-6">
      <header className="mb-6 flex items-end justify-between">
        <div>
          ...
          <p className="text-sm text-stone-500">
            Outstanding invoice balances by customer and bucket.
          </p>
        </div>
      ...
      {recon && (
        ...
        >
          {drift === 0 ? (
            <>
              Reconciliation OK. AR control{" "}
              <strong>{formatCents(recon.ar_control_balance_cents)}</strong>{" "}
              matches sub-ledger{" "}
              <strong>
                {formatCents(
                  recon.ar_sub_ledger_cents -
                    recon.ar_unapplied_credits_cents,
                )}
              </strong>{" "}
              (sub-ledger {formatCents(recon.ar_sub_ledger_cents)} \u2212
              unapplied credit{" "}
              {formatCents(recon.ar_unapplied_credits_cents)}).
            </>
          ) : (
            <>
              Reconciliation DRIFT: control ={" "}
              {formatCents(recon.ar_control_balance_cents)}, sub-ledger ={" "}
              {formatCents(recon.ar_sub_ledger_cents)}, unapplied ={" "}
              {formatCents(recon.ar_unapplied_credits_cents)}, difference ={" "}
              {formatCents(drift)}.
            </>
          )}
        </div>
